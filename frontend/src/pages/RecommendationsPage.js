...
import { ArrowLeft, Filter, Sun, Shield } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-[#FDFBF7]">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="plant-card p-6 mb-8">
          ...
          <div className="flex flex-wrap gap-4">
            <label className="flex items-center gap-2 cursor-pointer">
              <input type="checkbox" checked={filters.pet_friendly} onChange={(e) => setFilters({ ...filters, pet_friendly: e.target.checked })} className="w-4 h-4 text-[#3E6A4B] rounded" data-testid="filter-pet-friendly" />
              <span className="text-sm text-[#1A2E20]">Solo Pet-Friendly</span>
            </label>
            <Select value={filters.light || 'all'} onValueChange={(val) => setFilters({ ...filters, light: val === 'all' ? '' : val })}>
              <SelectTrigger className="w-48 bg-[#F3F5F1] border-none rounded-xl py-3 px-4 text-[#1A2E20] focus:ring-2 focus:ring-[#3E6A4B]/30" data-testid="filter-light">
        ...
        {loading ? (
          <div className="text-center py-12"><p className="text-[#8A9F8E]">Caricamento...</p></div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {recommendations.map((plant, index) => (
              <div key={plant.name} className="plant-card card-hover overflow-hidden" data-testid={`recommendation-card-${index}`}>
                ...
                <div className="p-4">
                  ...
                  <div className="flex flex-wrap gap-2 mb-4">
                    {plant.pet_friendly && <span className="badge-pet-friendly flex items-center gap-1"><Shield size={12} />Pet-Friendly</span>}
                    <span className="badge-light flex items-center gap-1"><Sun size={12} />{plant.light}</span>
                    <span className="text-xs px-3 py-1 rounded-full" style={{ background: '#E2E8E4', color: '#1A2E20' }}>{plant.difficulty}</span>
                  </div>
                </div>
              </div>
            ))}
