...
function RemindersPage() {
  ...
  const getIcon = (type) => {
    switch (type) {
      case 'water': return <Droplets size={20} className="text-[#1B6CA8]" />;
      case 'fertilizer': return <Sun size={20} className="text-[#B58500]" />;
      case 'pruning': return <Scissors size={20} className="text-[#3E6A4B]" />;
      case 'propagation': return <Scissors size={20} className="text-[#E07A5F]" />;
    ...
    return (
      <div className="space-y-4">
        {reminders.map((reminder, index) => (
          <div key={reminder.id} className="plant-card p-4 flex items-center justify-between" data-testid={`reminder-${index}`}>
            ...
            <label className="relative inline-flex items-center cursor-pointer">
              <input type="checkbox" checked={reminder.enabled} onChange={() => toggleReminder(reminder.id, reminder.enabled)} className="sr-only peer" data-testid={`toggle-reminder-${index}`} />
  ...
  return (
    <div className="min-h-screen bg-[#FDFBF7]">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">{renderContent()}</div>
    </div>
  );
}
